...
def checksum(id, buf, len_lo, len_hi):
    c0 = 0
    c1 = 0
    c0 = (c0 + id) & 0xff
    c1 = (c1 + c0) & 0xff
...
def wrap_packet( packet_id, payload ):
    ...
    buf.append(len_lo)          # packet size (2 bytes)
    buf.append(len_hi)
    buf.extend(payload)         # copy payload
    (cksum0, cksum1) = checksum( packet_id, payload, len_lo, len_hi)
    ...
    def read(self, ser):
        ...
        if self.state == 7:
            ...
            if len(input):
                ...
                (cksum0, cksum1) = checksum( self.pkt_id, self.payload, self.pkt_len_lo, self.pkt_len_hi );
